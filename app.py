...
def get_advisor_index():
    ...
    # Check for index marker file (more reliable than checking individual files)
    marker_file = "./storage/.index_ready"
    index_files = ['docstore.json', 'index_store.json', 'vector_store.json']
    has_all_files = all(os.path.exists(f"./storage/{f}") for f in index_files)
    
    if has_all_files and os.path.exists(marker_file):
        try:
