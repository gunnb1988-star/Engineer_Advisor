...
if st.session_state.get('show_guide_modal') and st.session_state.get('selected_guide'):
    ...
    # Full page modal with buttons inside
    st.markdown(f"""
    <style>
...
{guide['content']}
            ...
            <div class="modal-footer">
                <div style="flex: 1; text-align: center; font-size: 0.9rem; color: #666;">
                    Use buttons below to close or delete ↓
                </div>
            </div>
        </div>
    </div>
    ...
    with col2:
        ...
        with subcol1:
            if st.button("✖️ Close", key="close_modal", use_container_width=True, type="primary"):
                ...
                # Reset dropdown
                del st.session_state['guide_selector']
                st.rerun()
