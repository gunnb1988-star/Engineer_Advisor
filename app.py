...
if st.session_state.get('show_guide_modal') and st.session_state.get('selected_guide'):
    ...
    # Hide sidebar and create dark overlay
    st.markdown("""
    <style>
    ...
    # Use Streamlit components for the modal content
    st.markdown("## ")
    st.markdown("## ")
    ...
    with st.container():
        ...
        st.caption(f"👤 By: {guide['author']} • 📅 {display_date}")
        st.markdown("---")
        st.text(guide['content'])
        st.markdown("---")
        st.caption("↓ Use buttons below to close ↓")
