...
def get_advisor_index():
    ...
    os.makedirs("./manuals", exist_ok=True)
    os.makedirs("./storage", exist_ok=True)  # Ensure storage exists
    storage_path = "./storage"
    
    # Try to load existing index
    index_files = ['docstore.json', 'index_store.json', 'vector_store.json', 'graph_store.json']
    has_index = all(os.path.exists(f"./storage/{f}") for f in index_files[:3])  # Check for key files
    ...
    if has_index:
        try:
            ...
            # Corrupted - delete and rebuild
            st.warning(f"Index corrupted: {e}")
    ...
    try:
        ...
        if query_words:  # Only search if we have meaningful keywords
            for guide in guides:
                ...
                # Require at least 2 keyword matches (or 1 if query only has 1-2 keywords)
                required_matches = min(2, max(1, len(query_words) // 2))
