...
# Custom CSS - Style 1: Sleek Corporate
st.markdown("""
    <style>
    /* Global Styles */
    .main {
    ...
    /* Quick Fix Box (Yellow-Green) */
    .quick-fix-box {
        background: linear-gradient(135deg, #FFD700 0%, #8BC34A 100%);
        padding: 2rem;
        border-radius: 12px;
        margin: 1.5rem 0;
    ...
    /* Technical Solution Box (White with Green accent) */
    .tech-solution-box {
        background: white;
        padding: 2rem;
        border-radius: 12px;
...
if st.session_state.get('show_guide_modal') and st.session_state.get('selected_guide'):
    ...
    # Hide sidebar and header for clean view
    st.markdown("""
    <style>
    [data-testid="stSidebar"] {
    ...
    # Guide card with better contrast
    st.markdown(f"""
    <div style='
    ...
    '>
        <h1 style='color: white; margin: 0 0 0.5rem 0; font-size: 1.8rem;'>📝 {guide['title']}</h1>
        <p style='color: #f0f0f0; margin: 0; font-size: 0.95rem;'>
            👤 {guide['author']} &nbsp;&nbsp;•&nbsp;&nbsp; 📅 {display_date}
        </p>
    </div>
    """, unsafe_allow_html=True)
    
    # Content in clean white text area
    st.text_area(
        "Guide Content:",
        value=guide['content'],
        height=400,
    ...
    with col1:
        if st.button("✖️ Close", key="close_modal", use_container_width=True, type="primary"):
            ...
            st.session_state.pop('selected_guide', None)
            st.session_state.pop('guide_selector', None)
            st.rerun()
    
    with col2:
        if is_admin():
            if st.button("🗑️ Delete", key="delete_modal_guide", use_container_width=True):
