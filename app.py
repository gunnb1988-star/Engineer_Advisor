...
def load_quick_guides():
    """Load quick guides from JSON file"""
    guides_file = "./data/quick_guides.json"
...
def display_logo():
    ...
    if os.path.exists(logo_path):
        ...
        with col2:
            st.image(logo_path, width=None)  # Fixed deprecated warning
    else:
        st.markdown("""
...
with st.sidebar:
        ...
        with st.form("add_guide_form"):
            ...
            if submit_guide:
                if guide_title and guide_content:
                    ...
                    st.info("💡 Clear cache to include in searches")
                    st.session_state['show_add_guide'] = False
                    st.rerun()
                else:
                    st.error("❌ Title and content required")
    
    # Show existing guides
    guides = load_quick_guides()
    if guides:
        with st.expander(f"📚 View Guides ({len(guides)})"):
            for guide in guides:
                st.markdown(f"**{guide['title']}**")
                st.caption(f"By: {guide['author']} | {guide['created']}")
                st.text(guide['content'][:100] + "..." if len(guide['content']) > 100 else guide['content'])
    ...
    if is_admin():
        ...
        # --- BACKUP (Auto-saves to storage for persistence) ---
        st.subheader("📦 Backup")
        ...
        if st.button("💾 Create Backup"):
            ...
            if os.path.exists(storage_path) and os.listdir(storage_path):
                try:
                    ...
                    st.success(f"✅ Backup saved to storage/{backup_name}")
                    st.info("This backup persists on Streamlit Cloud!")
                except Exception as e:
                    st.error(f"❌ Backup failed: {e}")
            else:
                st.warning("⚠️ No storage data found")
...
def get_advisor_index():
    """Build or load the vector index with HIGH-QUALITY parsing"""
    ...
    try:
        # HIGH-QUALITY PARSING with premium features
        parser = LlamaParse(
            result_type="markdown",
            premium_mode=True,  # Enable premium parsing
            use_vendor_multimodal_model=True,  # Better image/table extraction
            vendor_multimodal_model_name="openai-gpt-4o",  # Best OCR model
            invalidate_cache=False,  # Use cache to save tokens
            do_not_cache=False,
            gpt4o_mode=True,  # Enable GPT-4O vision for images
...
query = st.text_input(
    ...
    help="Searches PDFs and quick guides",
    key="main_search"
)
...
if query:
    try:
        with st.spinner("🔍 Searching manuals & guides..."):
        ...
        if hasattr(response, 'source_nodes') and response.source_nodes:
            for idx, node in enumerate(response.source_nodes, 1):
                ...
                if source == 'quick_guides':
                    st.info(f"📝 Source {idx}: Quick Reference Guides")
                else:
                    ...
                    with st.expander(f"📄 Source {idx}: {file_name} (Page {page})"):
                        if hasattr(node, 'score'):
                            st.caption(f"**Relevance:** {node.score:.3f}")
