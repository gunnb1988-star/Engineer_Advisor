...
def get_user_role(username):
    ...
    try:
        ...
        st.error(f"Error getting role: {e}")
        return None
...
def get_advisor_index():
    ...
    try:
        with st.spinner(f"🔄 Building index from {len(pdf_files)} PDF file(s)..."):
            st.info("⏳ This may take several minutes depending on file size...")
...
# Search interface - SINGLE SEARCH INPUT
query = st.text_input(
...
if query:
    try:
        ...
        # Main answer with better readability
        st.markdown(
...
# Footer
st.markdown("---")
st.caption("Engineer Advisor • Powered by OpenAI & LlamaParse • Version 2.6")def save_custom_password(username, new_password_hash):
    """Save a custom password for a user"""
    password_file = "./data/custom_passwords.json"
