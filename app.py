...
def get_user_role(username):
    ...
    try:
        ...
        # Check if user exists in users dict or custom passwords
        users = st.secrets.get("users", {})
    ...
    if is_admin():
        ...
        with col2:
            if st.button("🔄 Rebuild Index"):
                ...
                if os.path.exists("./data/custom_passwords.json"):
                    os.remove("./data/custom_passwords.json")
                st.cache_resource.clear()
...
def get_advisor_index():
    ...
    if os.path.exists(storage_path) and len(os.listdir(storage_path)) > 0:
        try:
            ...
            # Silently delete corrupted storage and rebuild
...
# Build/load the index with a single loading message
